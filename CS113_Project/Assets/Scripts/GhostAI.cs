...
public class GhostAI : MonoBehaviour {
    ...
    void Update() {
        ...
        GameObject light = GameObject.FindWithTag("Flashlight");
        Vector3 player_pos = player.transform.position;
        Vector3 ghost_pos = transform.position;
 
        if (Vector3.Distance(player_pos, ghost_pos) <= range && Vector3.Angle(light.transform.forward, ghost_pos - player_pos) < angle && on_hit == false)
        {
